...
(function ($) {
  ...
  const checkPersistentElements = function (revealClass, vslId) {
    const storageKey = `vsl_revealed_${vslId}_${revealClass.replace(
      /\W/g,
      "_"
    )}`;
  ...
  const initOfferReveal = function () {
    $(".vsl-player-container").each(function () {
      ...
      const revealMode = $container.data("offer-reveal-mode") || "video";
      const revealTime =
        parseInt($container.data("offer-reveal-time"), 10) || 0;
      ...
      // Setup reveal based on mode
      if (revealMode === "page") {
        // Page time mode: reveal after X seconds on page
        setupPageTimeReveal(revealClass, revealTime, persist, vslId);
      } else {
        // Video time mode: reveal after X seconds of video watched
  ...
  // Setup page time-based reveal
  const setupPageTimeReveal = function (
    revealClass,
    revealTime,
    persist,
    vslId
    ...
    // Set timeout to reveal after specified time
    setTimeout(function () {
      revealElements(revealClass, persist, vslId);
    }, revealDelay);
  };
