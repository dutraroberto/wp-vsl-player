...
(function ($) {
  ...
  function vslResumeLog(message, data) {
    console.log(
      "%c[VSL Resume] " + message,
      "background: #4a6da7; color: white; padding: 2px 5px; border-radius: 3px;",
      data || ""
    );
  }
  ...
  function initializeResumeFeature() {
    ...
    $(".vsl-player-container").each(function () {
      ...
      var enableResumePlayer =
        $container.data("enable-resume-player") === true ||
        $container.data("enable-resume-player") === "true";

      if (containerId && vslId) {
        if (enableResumePlayer) {
          vslResumeLog("Player habilitado para retomada:", {
            containerId: containerId,
            vslId: vslId,
          });
          // Verificar se existe um tempo salvo
          var savedTime = getSavedTime(vslId);
          vslResumeLog("Tempo salvo encontrado:", {
            vslId: vslId,
            savedTime: savedTime,
          });

          if (savedTime && savedTime > 0) {
            vslResumeLog("Preparando overlay de retomada para o player", {
              containerId: containerId,
              vslId: vslId,
              savedTime: savedTime,
            });
    ...
    $(document).on(
      ...
      function (event, player, scriptId) {
    ...
    $(document).on(
      "YT.PlayerState.ENDED.resumePlayer",
      function (event, player, scriptId) {
        var playerId = "vsl-player-" + scriptId;
        var $container = $("#" + playerId);
        var vslId = $container.data("vsl-id");
        ...
        vslResumeLog("Vídeo terminou, limpando dados de retomada", {
          scriptId: scriptId,
          vslId: vslId,
        });
  ...
  function createResumeOverlay($container, containerId, vslId, savedTime) {
    ...
    var resumeOverlayColor =
      $container.data("resume-overlay-color") || "#000000";
    var resumeButtonColor = $container.data("resume-button-color") || "#617be5";
    var resumeButtonHoverColor =
      $container.data("resume-button-hover-color") || "#3854c3";

    // Criar o elemento do overlay
    var $resumeOverlay = $(
      '<div class="vsl-resume-overlay">' +
    ...
    $resumeOverlay
      .find(".vsl-resume-continue")
      .css("background-color", resumeButtonColor);
    ...
    $resumeOverlay.find(".vsl-resume-continue").on("click", function () {
      ...
      if (window.VSL_Player_Interaction) {
        vslResumeLog(
          "Notificando analytics sobre a ação de continuar assistindo",
          { action: "continue", time: savedTime }
        );
  ...
  function initializeVideoPlayer($container, containerId, vslId, startTime) {
    vslResumeLog("Inicializando player de vídeo", {
      containerId: containerId,
      vslId: vslId,
      startTime: startTime,
    });
    ...
    vslResumeLog("Verificação de player existente:", {
      playerExists: !!player,
    });

    if (player) {
      // O player já existe, apenas definir o tempo de início e reproduzir
      vslResumeLog("Player já existe, definindo tempo e iniciando", {
        startTime: startTime,
      });
      ...
      vslResumeLog("Player não existe, disparando evento de inicialização", {
        vslId: vslId,
        startTime: startTime,
      });
      $(document).trigger("VSL.InitializePlayer", [
        $container,
        vslId,
        startTime,
      ]);
      ...
      vslResumeLog(
        "Iniciando verificação periódica de inicialização do player"
      );
  ...
  function getSavedTime(vslId) {
    ...
    vslResumeLog("Buscando tempo salvo", {
      key: savedTimeKey,
      value: savedTime,
    });
