...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(document).on(
      "YT.PlayerState.ENDED.progressBar",
      function (e, player, scriptId) {
        console.log(
          "[Progress Bar] Vídeo terminou, resetando barra de progresso para " +
            scriptId
        );
    ...
    $(document).on(
      "click",
      ".vsl-start-overlay, .vsl-pause-overlay",
      function () {
  ...
  function initProgressBar(player, scriptId) {
    ...
    if ($container.find(".vsl-progress-container").length === 0) {
      $container.append(
        '<div class="vsl-progress-container"><div class="vsl-progress-bar"></div></div>'
      );
    }
    ...
    if (
      typeof VSLProgressBar !== "undefined" &&
      VSLProgressBar.progress_color
    ) {
      progressBars[scriptId].bar.css(
        "background-color",
        VSLProgressBar.progress_color
      );
    }
